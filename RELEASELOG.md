...
- **西班牙语本地化（es）** — 社区贡献 @ZoeySigel（#312 完整翻译 14 个 locale 文件），随后补齐 rc.0 新增的 162 个 key（#325），关键不变量全部通过：key 完整性零缺失、`{{var}}` / i18next 复数占位符零丢失
- **data-analyst 角色模板**（#310）— 新增缺失的数据分析师 Agent 模板，含职责表、5 步工作流、指标契约与验收标准；引用的 12 个工具逐一核对真实存在
- **release-notes-workflow 示例**（#311）— 跑在真实 WorkflowEngine 上的发布说明生成示例：git log 解析 → writer/reviewer 双 agent → 独立审查步 → 拒绝覆盖已有文件；配套 `example:release-notes` 脚本
- **README 全面重写** — 更短更酷：自举（Markus 由 Markus 开发）为最大卖点，面向独立开发者与小团队；同步 0.10.0 能力（真正并发 / ContextOS / 模型统一发现）

### Housekeeping

- 清理积压：6 个被新架构取代或已被 main 实现的旧 PR 关闭归档（逐一留技术理由）；2 个无效/已解决 issue 关闭；#294（Tasks 页面显示）已请求报告者升级 0.10.x 验证

### Stats

- 11 个提交（自 `v0.10.0-rc.0`），其中 6 个来自社区贡献
...
- **无边框 UI 改版** — 全局无边框设计语言，DAG 布局改进，动态 handle 位置
- **Agent 概览/文件编辑器/侧边栏重设计** — Agent 页面视觉重构，面板分割器优化
- **Team 页面重设计** — L2 面板布局，扁平化标签页，行内编辑，完善 i18n
- **DM 聊天统一布局** — DM 聊天采用频道式布局，输入框样式优化
- **Mailbox 深度审议决策** — mailbox 决策升级为完整审议流程，修复 notify_user 可见性与场景提示
- **多格式交付物输出** — 支持 Markdown、HTML、JSON 等多种交付物格式
- **Bocha 搜索后端** — 新增 Bocha 搜索引擎，移除效果不佳的 Bing 搜索；搜索 API Key 可在设置中配置
- **LLM 自动降级开关** — 新增模型自动 fallback 切换控制
- **语义重试循环检测** — 新增 sameToolBurst 检测器，捕获 Agent 语义重试死循环

### Bug Fixes

- **修复持久 Shell 会话** — Shell 会话持久化，清理工具输出中的 LLM 污染内容，修复前端 OOM
- **修复 MCP 断连自动重连** — 工具调用发现 MCP 断连时自动重连；Agent 会话结束时回收 MCP 进程
- **修复审批按钮逻辑** — 重构审批按钮：修复自由文本输入始终走审批通过的 bug，任务/需求增加结构化审批模式
- **修复人工审核回调** — 防止 human review HITL 回调将已完成任务回退到 in_progress
- **修复 notify_context 泄漏** — 聊天消息中剥离 HTML 注释，正确显示通知徽章
- **修复 Report 页面指标** — 使用真实模型定价计算成本，移除不可靠的计费数据
- **修复 notify/escalation 事件转发** — 正确转发事件，启动时等待健康检查后再打开浏览器
- **修复聊天流式卡住** — 修复 thinking 状态卡死，Dashboard UX 改进，目录交付物预览
- **修复 inline-completed 语义** — 修正内联完成项语义，补充缺失的活动日志
- **修复亮色模式可读性** — Markdown 和表格在亮色模式下样式修复

### Improvements

- **侧边栏/通知/团队聊天列表优化** — 视觉与交互细节改进
- **面板布局优化** — 详情面板全宽填充，滚动按钮位置调整避免遮挡发送按钮
- **通知/审批内容展示** — 移除 Markdown 内容最大高度限制
- **公告/规范编辑器** — 编辑器填满整个视口高度
- **Desktop 布局呼吸感** — 面板间距统一，Team L2 浮动模式，Work 标签宽度一致

### Stats

- 92 files changed, +5,246 / −1,556 lines
...
- **模型提供商一键激活** — 主流提供商（Anthropic、OpenAI、Google Gemini 等）无需填写 Base URL、上下文窗口、定价或模型名称，点击提供商后粘贴 API Key 即可使用，内置模型目录自动提供所有元数据；保留"Add Provider"入口供自定义提供商
- **添加智能体/新建团队方式选择** — 点击"添加智能体"或"新建团队"时弹出选择弹窗，用户可选择让秘书帮忙（推荐）或手动操作；下拉菜单顺序调整为：添加智能体 → 新建团队 → 添加成员 → 新建群聊

### Bug Fixes

- **修复聊天输入框不自动伸缩** — 提取 textarea 高度调整为可复用回调 + useEffect，输入框随长内容自动增高，清空或发送后自动缩回
