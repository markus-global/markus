...
| 架构深度分析 | [architecture-analysis.md](./architecture-analysis.md) |
...
| Agent 治理框架 | [ARCHITECTURE.md — §治理框架](./ARCHITECTURE.md#十治理框架governance-framework) |
| API 参考概览 | [api-reference-guide.md](./api-reference-guide.md) |
...
| 文档 | 说明 | 维护者 |
|------|------|--------|
| [ARCHITECTURE.md](./ARCHITECTURE.md) | 系统架构、包结构、数据流、治理框架 | 架构 |
| [architecture-analysis.md](./architecture-analysis.md) | 基于源码的深度架构分析报告 | 架构 |
...
| Agent 行为规范 | [../templates/roles/SHARED.md](../templates/roles/SHARED.md) | — |

### 4. API 参考

完整 API 定义在 `packages/org-manager/src/api-server.ts`。自动生成的 TypeDoc 文档运行 `pnpm typedoc` 后可在 `docs/api/` 查看（已 gitignore）。
...
| `/api/system/emergency-stop` | POST | 紧急停止所有 Agent |
...
```
packages/
├── shared/       — 共享类型与工具函数（含治理/项目/知识库类型）
├── core/         — Agent 运行时、记忆、LLM、工具、工作区管理
├── storage/      — PostgreSQL 持久层 (Drizzle ORM)
├── org-manager/  — 组织管理、API 服务器、任务系统、治理服务
├── a2a/          — Agent-to-Agent 协议
├── web-ui/       — React Web UI（含治理/项目/知识库/报告页面）
├── cli/          — 命令行入口 + 服务组装
