...
> Last updated: 2026-09（v0.9.9 发版评估后的常青基线）。

- **后端阈值**（`packages/core` / `org-manager` / `storage` / 合计）：**75 / 65 / 78 / 80**。
  全仓约 **360 个测试文件**，后端测试扎实，是发版的可信前提。
- **CI 门禁构成**：架构门禁 3 条规则、会话不变量单独成步、覆盖率棘轮（只升不降）、
  HOME 隔离（`MARKUS_*` 环境变量未隔离——已知缺口）。
- **前端覆盖率历史教训**：曾被整体挡在门外（vitest include 不含 `.tsx`、exclude 排除
  `packages/web-ui`），表现为假 0%——是**分母装错**，不是收集坏了。前端需显式收集
  `.tsx` 并配置独立的 coverage include。
- **缺口清单**：前端约 20 个高风险零测试文件（具体清单随时间变化，以 CI 缺口报告为准）。

### 5.2 测试质量规范（防假绿）

- **假设绿需反向验证**：测试通过不等于测试有效——故意引入一个 bug，确认它变红。
- **行为级断言 > 文本嗅探**：断言函数/语义结果，不要断言输出文本片段。
- **环境隔离**：测试不得依赖宿主机环境变量；CI 的 wui-check / 独立配置要显式隔离。
- **跳过用例需审计**：`it.skip` / `describe.skip` 要有理由，否则视为欠账。
- **覆盖率棘轮**：只升不降；防止 0% 假绿 / 静默失效（覆盖率配置死了要能报警）。
- **真实缺陷模式备忘**（往期修复，防复发）：Gemini 推理内容静默丢失、事务异步回调
  不回滚、provider 超时无兜底、watchdog 无信号——修复时请回归这些模式。

### 5.3 vitest 多 project 配置注意事项

- `coverage.include` / `exclude` 是 **project 级**配置，根级设置不自动继承到子 project——
  漏配会让子 project 覆盖率静默为 0 或跑错文件集（前端假 0% 的根因之一）。
- 新增一个 project 时，必须同时核对：include 是否覆盖目标扩展名（`.tsx`）、
  exclude 是否误杀目标包、coverage 门禁是否生效（故意改低阈值验证会红）。
