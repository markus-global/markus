...
│组织    │ │任务     │ │Agent    │ │Service │ │Report·Knowledge │
│管理    │ │+ 审批   │ │生命周期 │ │迭代管理│ │Trust·Archive    │
└──┬────┘ └──┬─────┘ └──┬──────┘ └┬───────┘ └┬────────────────┘
   │         │          │         │           │
┌──▼─────────▼──────────▼─────────▼───────────▼───────────────┐
...
| `junior` | score ≥ 40, 交付 ≥ 3 次 | 标准任务需 Manager 审批 |
| `standard` | score ≥ 60, 交付 ≥ 10 次 | 常规任务可自动审批 |
| `senior` | score ≥ 80, 交付 ≥ 25 次 | 高度自治，可评审他人 |
...
Organization (Org)
 ...
 ├── Projects — 有仓库、治理规则、迭代的工作范围
 │    ├── Iterations (Sprint/Kanban) — 任务容器
...
- 每个 Task 归属于一个 Project 和 Iteration
...
| `project_info` / `iteration_status` | 查询项目和迭代信息 |
| `knowledge_contribute` / `knowledge_search` / `knowledge_browse` / `knowledge_flag_outdated` | 项目知识库操作 |
...
| 迭代报告 | 迭代结束时 | 迭代复盘、carry-over 项、下期计划 |

**计划审批流程：** 周报/迭代报告中的工作计划需人工审批 → 审批通过后自动创建任务 → Agent 不得在计划审批前开始工作
...
**设计原则：**
- Agent 需要**决策**依据的 → 放入 Context（项目目标、迭代时间线、治理规则）
- Agent 需要**主动操作**的 → 做成 Tool（提交评审、查询项目信息、贡献知识）
