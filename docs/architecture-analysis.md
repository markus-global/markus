...
| 技能工具   | `SkillRegistry`              | git*\*, code_analysis*_, browser\__                                 |
| MCP 工具   | `MCPClientManager`           | 外部 MCP 服务器提供的工具                                           |
| 沙箱工具   | Sandbox override             | sandboxed*shell, sandboxed_file*\*                                  |
...
## 10. Agent 治理框架（Governance Framework）

> 新增于 v0.8.0 | 解决多 Agent 并发运行时的治理、隔离、交付和审计问题

### 10.1 问题背景

1. **不受控的任务创建**：Team Lead Agent 通过 `task_create` 自由创建大量任务，无审批流程
2. **无全局控制**：缺乏一键暂停/紧急停止所有 Agent 的能力
3. **无系统公告**：无法向所有 Agent 广播策略变更或指令
4. **工作区冲突**：所有 Agent 共享同一仓库路径，并发修改代码互相覆盖
5. **无正式交付流程**：任务从 in_progress 直接跳到 completed，无交付物、无评审
6. **无项目/迭代概念**：Agent 默认只工作于 Markus 本身，无法引导至其他项目

### 10.2 架构设计

#### 项目与迭代系统

- **Project**：有作用域的工作主体，关联 0-N 个代码仓库、多个团队、独立治理策略
- **Iteration**：项目内的工作容器（Sprint 或 Kanban 模型）
- 核心文件：`packages/org-manager/src/project-service.ts`
- DB 表：`projects`、`iterations`

#### 任务治理策略

- 三层审批：`auto`（低优先级子任务）→ `manager`（标准任务）→ `human`（高优先级/影响共享资源）
- 代理信任等级影响实际审批层级：`probation` 始终需要人工审批，`senior` 可跳过 manager 审批
- 核心文件：`packages/org-manager/src/task-service.ts` (governance 部分)、`packages/org-manager/src/trust-service.ts`

#### 全局 Agent 控制

- `AgentManager.pauseAllAgents()` / `resumeAllAgents()` / `emergencyStop()`
- Agent 新增 `pause()` / `resume()` 方法
- API：`POST /api/system/pause-all`、`POST /api/system/resume-all`、`POST /api/system/emergency-stop`

#### 系统公告

- `AgentManager.broadcastAnnouncement()` 通过 A2A 总线 + WebSocket 广播
- 注入到 `ContextEngine.buildSystemPrompt()` 的 `## System Announcements` 区域
- DB 表：`system_announcements`

#### 工作区隔离（Git Worktree）

- `WorkspaceManager`：每个任务在项目仓库中创建独立的 git worktree
- 路径格式：`<repo>/.worktrees/task-<taskId>`，分支：`task/<taskId>`
- 任务完成后的交付物即为该分支上的提交
- 核心文件：`packages/core/src/workspace-manager.ts`

#### 正式交付与评审流程

- 新任务状态：`review`、`revision`、`accepted`、`archived`
- `TaskDeliverable` 类型：branch、file、document、report
- 工具：`task_submit_review` 替代直接标记完成
- 质量门禁：提交评审前自动运行 TypeScript 检查、测试、lint

#### 项目知识库

- 三层知识作用域：`personal`（个人记忆）、`project`（项目共享）、`org`（组织级）
- 知识分类：architecture、convention、api、decision、gotcha、troubleshooting 等
- 新工具：`knowledge_contribute`、`knowledge_search`、`knowledge_browse`、`knowledge_flag_outdated`
- 核心文件：`packages/org-manager/src/knowledge-service.ts`
- DB 表：`project_knowledge`

#### 周期性报告与计划审批

- `ReportService`：自动生成日报/周报/月报，聚合任务、计费、审计、知识数据
- 计划审批流：周报/迭代报告中的计划需人工审批后才自动创建任务
- 人类反馈机制：对报告的批注/评论/指令可定向或广播给 Agent
- 核心文件：`packages/org-manager/src/report-service.ts`
- DB 表：`reports`、`report_feedback`

#### Agent 感知策略（三层模型）

| 层级                      | 文件                                  | 作用                                                                                                               |
| ------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| SHARED.md（静态规范）     | `templates/roles/SHARED.md`           | 所有 Agent 共享的行为规范：项目工作、任务治理、工作区纪律、正式交付、知识管理、报告                                |
| ContextEngine（动态注入） | `packages/core/src/context-engine.ts` | 每次交互动态注入：项目上下文、当前工作区、系统公告、人类反馈、项目知识                                             |
| Tools（机械执行）         | `packages/core/src/tools/`            | 强制执行层：`task_create` 阻塞等待审批、`task_submit_review` 替代直接完成、shell/file 工具自动限制在 worktree 路径 |

### 10.3 新增关键文件

| 文件                                            | 职责                        |
| ----------------------------------------------- | --------------------------- |
| `packages/shared/src/types/governance.ts`       | 治理相关共享类型            |
| `packages/shared/src/types/project.ts`          | 项目/迭代/报告/知识共享类型 |
| `packages/core/src/workspace-manager.ts`        | Git Worktree 管理           |
| `packages/core/src/tools/project-tools.ts`      | 项目/迭代/知识查询工具      |
| `packages/org-manager/src/project-service.ts`   | 项目与迭代 CRUD             |
| `packages/org-manager/src/knowledge-service.ts` | 项目知识库管理              |
| `packages/org-manager/src/report-service.ts`    | 周期性报告生成与计划审批    |
| `packages/org-manager/src/trust-service.ts`     | Agent 信任等级管理          |
| `packages/org-manager/src/archive-service.ts`   | 归档与生命周期清理          |
| `packages/org-manager/src/stale-detector.ts`    | 停滞检测与自动升级          |

### 10.4 新增 API 端点

| 端点                            | 方法           | 说明                      |
| ------------------------------- | -------------- | ------------------------- |
| `/api/system/pause-all`         | POST           | 暂停所有 Agent            |
| `/api/system/resume-all`        | POST           | 恢复所有 Agent            |
| `/api/system/emergency-stop`    | POST           | 紧急停止                  |
| `/api/system/status`            | GET            | 全局状态（暂停/紧急模式） |
| `/api/system/announcements`     | GET/POST       | 系统公告 CRUD             |
| `/api/projects`                 | GET/POST       | 项目列表/创建             |
| `/api/projects/:id`             | GET/PUT/DELETE | 项目详情/更新/删除        |
| `/api/projects/:id/iterations`  | GET/POST       | 迭代列表/创建             |
| `/api/iterations/:id/status`    | PUT            | 更新迭代状态              |
| `/api/tasks/:id/accept`         | POST           | 接受任务交付              |
| `/api/tasks/:id/revision`       | POST           | 请求修订                  |
| `/api/tasks/:id/archive`        | POST           | 归档任务                  |
| `/api/governance/policy`        | GET/PUT        | 治理策略查看/更新         |
| `/api/reports`                  | GET            | 报告列表                  |
| `/api/reports/generate`         | POST           | 手动触发报告生成          |
| `/api/reports/:id`              | GET            | 报告详情                  |
| `/api/reports/:id/plan/approve` | POST           | 审批计划                  |
| `/api/reports/:id/plan/reject`  | POST           | 拒绝计划                  |
| `/api/reports/:id/feedback`     | GET/POST       | 报告反馈 CRUD             |
| `/api/knowledge`                | POST           | 贡献知识                  |
