...
### 6.8 Phase 6 — 上下文质量与实战验证（下一阶段 🔜）

**目标**：解决 Agent 实际可用性的根本瓶颈，让 Agent 真正能干活。

> **第一性原理**：`Agent 有用性 = P(正确决策 | 上下文) × P(行动成功 | 决策)`。经过 Phase 0-5，工具可靠性（右项）已达可用水平。瓶颈转移到上下文质量（左项）、创作体验、和社区生态。

| 优先级 | 任务               | 说明                                                          | 验收标准                                               |
| ------ | ------------------ | ------------------------------------------------------------- | ------------------------------------------------------ |
| **P0** | 记忆向量语义检索   | 一步到位引入 pgvector，`memory_search` 改为向量余弦相似度检索 | 语义检索召回率 ≥ 70%（"处理超时" 匹配 "加了 timeout"） |
| **P0** | Token 计量精准化   | 三层策略：API 实际值 → Anthropic counting API → 本地 tiktoken | pre-flight 估算误差 < 15%                              |
| **P0** | Agent 运行环境感知 | 启动时自动探测 OS/工具/浏览器/运行时，注入系统提示            | Agent 不再尝试调用不存在的工具                         |
| **P1** | Builder 重新设计   | 引导式创建 + 实时预览 + 完整配置（LLM/工具/预算/环境需求）    | 新用户可视化创建可工作的 Agent                         |
| **P1** | 端到端任务验证     | 5 个标准化验证场景，用真实 Agent 执行并修复失败模式           | ≥ 3 个场景 Agent 可独立完成                            |
| **P1** | 快速启动体验       | `markus init` 交互式引导 + 零配置模式（无 DB 时 SQLite）      | 新用户 5 分钟内启动首个 Agent                          |
| **P2** | 社区 GitHub 模式   | 模板/技能支持公开/私有/Fork/版本历史/上游同步                 | 用户可 Fork 公开模板并定制                             |
| **P2** | 上下文精细化       | 上下文段落优先级权重 + 角色提示词基于实测优化                 | 128K 窗口利用率 ≥ 85%                                  |

**砍掉的原 Phase 6 项目**（理由详见 `docs/architecture-analysis.md` 第 8.1 节）：

- ~~Cross-Org Federation~~ — 无多组织场景
- ~~Multi-Tenant Deployment~~ — 单组织 OSS 部署
- ~~Agent 克隆与迁移~~ — 无跨环境需求
- ~~分布式任务执行~~ — 单进程足够
- ~~GUI 自动化深度集成~~ — 非核心路径
...
- Prompt Engineering Studio — 等 P2 角色提示词优化完成后再评估可视化需求
...
| Phase 6 P0 | 记忆向量检索召回率                | ≥ 70%            | 🔲 待开发            |
| Phase 6 P0 | Token pre-flight 估算误差         | < 15%            | 🔲 待开发            |
| Phase 6 P0 | 环境探测覆盖率                    | ≥ 90% 常用工具   | 🔲 待开发            |
| Phase 6 P1 | Builder 可视化创建成功率          | ≥ 90%            | 🔲 待开发            |
| Phase 6 P1 | 端到端任务完成率                  | ≥ 60% (3/5 场景) | 🔲 待开发            |
| Phase 6 P1 | 新用户启动时间                    | < 5 min          | 🔲 待开发            |
| Phase 6 P2 | 模板 Fork 功能可用                | 100%             | 🔲 待开发            |
...
### 9.2 Phase 6 — Agent 治理框架（已实现）

解决多 Agent 并发运行时的关键治理问题：不受控任务创建、无工作区隔离、无正式交付流程。

#### 已完成功能

| 类别       | 功能                                         | 关键文件                                             |
| ---------- | -------------------------------------------- | ---------------------------------------------------- |
| 全局控制   | 一键暂停/恢复/紧急停止所有 Agent             | `agent-manager.ts`, `agent.ts`                       |
| 系统公告   | 广播消息给所有 Agent + 注入提示词上下文      | `agent-manager.ts`, `context-engine.ts`              |
| 项目系统   | Project/Iteration CRUD、团队-项目关联        | `project-service.ts`, schema `projects`/`iterations` |
| 任务治理   | 三层审批（auto/manager/human）+ 信任等级调节 | `task-service.ts`, `trust-service.ts`                |
| 工作区隔离 | Git Worktree per task、分支隔离              | `workspace-manager.ts`                               |
| 正式交付   | review/revision/accepted 状态、交付物提交    | `task-service.ts`, `task-tools.ts`                   |
| 质量门禁   | 提交评审前自动 typecheck/lint/test           | `review-service.ts`                                  |
| 项目知识库 | 三层知识作用域、分类搜索、贡献/标记过期      | `knowledge-service.ts`, schema `project_knowledge`   |
| 周期报告   | 自动生成日报/周报/月报 + 计划审批            | `report-service.ts`, schema `reports`                |
| 人类反馈   | 报告批注/评论/指令 → Agent 上下文注入        | `report-service.ts`, `context-engine.ts`             |
| Agent 感知 | SHARED.md 规范 + 动态上下文注入 + 工具强制   | `SHARED.md`, `context-engine.ts`, `project-tools.ts` |
| 成本归因   | 按 task/project 统计用量和费用               | `billing-service.ts`                                 |
| 归档清理   | 自动归档完成任务、清理 worktree/branch       | `archive-service.ts`                                 |
| 停滞检测   | 卡住的任务/评审/迭代自动预警                 | `stale-detector.ts`                                  |
| 审计持久化 | 所有治理事件写入 audit_logs 数据库           | `audit-service.ts`                                   |

#### 新增 API 端点

- `POST /api/system/pause-all` / `resume-all` / `emergency-stop` — 全局 Agent 控制
- `GET/POST /api/system/announcements` — 系统公告
- `GET/POST /api/projects` + `/:id` — 项目 CRUD
- `GET/POST /api/projects/:id/iterations` — 迭代管理
- `POST /api/tasks/:id/accept` / `revision` / `archive` — 评审流程
- `GET/PUT /api/governance/policy` — 治理策略
- `GET/POST /api/reports` + `/:id/plan/approve` / `reject` / `feedback` — 报告与计划审批
- `GET/POST /api/knowledge` — 知识库

### 9.3 下一阶段优先级（Phase 7 — 上下文质量与实战验证）

基于第一性原理分析，瓶颈已从工具可靠性转移到上下文质量、创作体验和社区生态：

1. **P0 记忆向量检索**：一步到位引入 pgvector 语义检索——影响所有 Agent 所有交互
2. **P0 Token 精准计量**：三层策略（API 实际值 → counting API → tiktoken），中文场景可多利用 ~40% 上下文
3. **P0 运行环境感知**：Agent 启动时自动探测 OS/工具/浏览器，避免盲目操作
4. **P1 Builder 重设计**：引导式创建 + 实时预览 + 完整配置（LLM/工具/预算/环境需求）
5. **P1 端到端任务验证**：用真实编码任务测试 Agent，修复发现的失败模式
6. **P1 快速启动体验**：`markus init` + 零配置模式，5 分钟内启动首个 Agent
7. **P2 社区 GitHub 模式**：模板/技能支持公开/私有/Fork/版本/上游同步
8. **P2 上下文精细化**：窗口优先级管理 + 角色提示词基于实测数据优化

### 9.4 执行原则

- **瓶颈优先**：只做能提升 `P(正确决策 | 上下文)` 的工作，拒绝"看起来重要但没人用"的功能
