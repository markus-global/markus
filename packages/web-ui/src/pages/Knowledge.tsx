...
const CATEGORY_ICONS: Record<string, string> = {
  architecture: '\u{1F3D7}', convention: '\u{1F4CF}', api: '\u{1F50C}', decision: '\u{2696}',
  gotcha: '\u{26A0}', troubleshooting: '\u{1F527}', dependency: '\u{1F4E6}', process: '\u{1F504}', reference: '\u{1F4DA}',
};
...
export function KnowledgePage() {
  ...
  const handleContribute = async () => {
    ...
    try {
      ...
      setNewTitle(''); setNewContent(''); setNewTags(''); setNewImportance(5);
      ...
      <div className="w-96 border-r border-gray-800 flex flex-col bg-gray-950 shrink-0">
        <div className="p-4 border-b border-gray-800 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setShowContribute(true)} className="text-xs px-2.5 py-1 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white transition-colors">+ Contribute</button>
          </div>
          <input
            value={searchQuery}
          ...
          <div className="flex gap-1.5 flex-wrap">
            <FilterPill label="All" value="" current={filterScope} onClick={setFilterScope} />
            {SCOPES.map(s => <FilterPill key={s} label={s} value={s} current={filterScope} onClick={setFilterScope} />)}
          </div>
          <div className="flex gap-1.5 flex-wrap">
            <FilterPill label="All categories" value="" current={filterCategory} onClick={setFilterCategory} />
            {CATEGORIES.map(c => (
              <FilterPill key={c} label={`${CATEGORY_ICONS[c] ?? ''} ${c}`} value={c} current={filterCategory} onClick={setFilterCategory} />
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {loading ? (
            ...
            <EmptyState hasFilters={!!(debouncedQuery || filterScope || filterCategory)} onContribute={() => setShowContribute(true)} />
          ) : entries.map(entry => (
            <button
              key={entry.id}
            ...
            >
              ...
              <div className="flex items-center gap-2 mt-1.5">
                ...
                <span className="text-[10px] text-gray-500">{CATEGORY_ICONS[entry.category] ?? ''} {entry.category}</span>
          ...
          <div className="p-6 max-w-3xl space-y-5">
            <div>
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                <span className="px-2 py-0.5 rounded text-xs bg-gray-700 text-gray-400">{CATEGORY_ICONS[selected.category] ?? ''} {selected.category}</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => !contributing && setShowContribute(false)}>
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 w-[36rem] space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <div>
              ...
              <input value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="Concise, descriptive title"
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 focus:border-indigo-500 focus:outline-none transition-colors" />
            </div>

            <div>
              <label className="text-xs text-gray-500 block mb-1">Content * <span className="text-gray-600">(Markdown supported)</span></label>
              <textarea value={newContent} onChange={e => setNewContent(e.target.value)} placeholder="Detailed knowledge content..."
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 h-40 resize-none focus:border-indigo-500 focus:outline-none transition-colors font-mono" />
            </div>
            ...
            <div className="flex gap-3">
              ...
              <div className="w-28">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-300 w-5 text-right">{newImportance}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button onClick={() => setShowContribute(false)} disabled={contributing}
                className="text-sm text-gray-500 hover:text-gray-300 disabled:opacity-50 transition-colors">Cancel</button>
              <button onClick={handleContribute} disabled={contributing || !newTitle.trim() || !newContent.trim()}
