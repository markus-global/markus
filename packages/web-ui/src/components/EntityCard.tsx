...
export function EntityChip({ id, type: typeHint, label }: { id: string; type?: EntityType; label?: React.ReactNode }) {
  ...
  const handleClick = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    // Deliverables open in the right-side panel when the active page hosts one.
    if (meta.type === 'deliverable' && meta.deliverable && layout?.hostAvailable) {
      layout.openRightPanel({ kind: 'deliverable', deliverable: meta.deliverable });
      return;
    }
    navigateToEntity(id);
  };
...
export function EntityCard({ id, type: typeHint, label }: { id: string; type?: EntityType; label?: string }) {
  ...
  const handleClick = () => {
    if (meta.type === 'deliverable' && meta.deliverable) {
      // Prefer the right-side panel when the active page hosts one; else modal.
      if (layout?.hostAvailable) { layout.openRightPanel({ kind: 'deliverable', deliverable: meta.deliverable }); return; }
