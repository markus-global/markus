...
interface Props {
  ...
  submitting?: boolean;
  /** When true, the modal shows the previously submitted answer and cannot be re-submitted. */
  readOnly?: boolean;
  onClose: () => void;
  onSubmit: (result: {
    approved: boolean;
...
export function UserInputModal({ approval, submitting, readOnly, onClose, onSubmit }: Props) {
  const { t } = useTranslation(['team', 'common']);
  useNativeBrowserOverlay(true);
  const questions = useMemo(() => deriveQuestions(approval), [approval]);
  const [step, setStep] = useState(0);
  const [drafts, setDrafts] = useState<Record<string, DraftAnswer>>(() => {
    // In read-only mode, pre-fill drafts with the answer the user already submitted.
    if (readOnly && approval.answers && approval.answers.length > 0) {
      const initial: Record<string, DraftAnswer> = {};
      for (const ans of approval.answers) {
  ...
  const toggleOption = (q: UserInputQuestion, optId: string) => {
    ...
    if (q.allowMultiple) {
      ...
      setDraft(q.id, {
        selectedOptionIds: has ? d.selectedOptionIds.filter(x => x !== optId) : [...d.selectedOptionIds, optId],
        // Choosing an option means freeform is an alternative, not a dual answer.
        text: has ? d.text : '',
      });
    } else {
      // Re-click selected option to deselect (so the user can type freeform instead).
      const has = d.selectedOptionIds.includes(optId);
      setDraft(q.id, {
        selectedOptionIds: has ? [] : [optId],
        // Selecting a choice clears freeform; deselecting keeps any typed text.
        text: has ? d.text : '',
      });
    }
  };
    ...
    >
      <div className="w-full max-w-2xl max-h-[85vh] flex flex-col bg-surface-secondary border border-border-default rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-start gap-3 px-6 py-4 border-b border-border-default shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <p className="text-xs text-fg-tertiary">{approval.agentName}</p>
              {readOnly && (
                <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-green-500/15 text-green-600">
                  ...
                  {t('team:userInput.submitted', { defaultValue: 'Submitted' })}
                </span>
              )}
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
            ...
            <div className="space-y-2">
              {current.options.map((opt, idx) => {
                ...
                return (
                  <button
                    key={opt.id}
                    onClick={readOnly ? undefined : () => toggleOption(current, opt.id)}
                    disabled={readOnly}
                    className={`w-full px-4 py-3 text-left rounded-xl border transition-colors flex items-start gap-3 ${
                      selected
                        ? 'border-brand-500 bg-brand-500/10'
                        : `border-border-default ${readOnly ? '' : 'hover:bg-surface-overlay'}`
                    } ${readOnly ? 'cursor-default' : ''} ${readOnly && !selected ? 'opacity-50' : ''}`}
                  >
          ...
          {(current.inputType === 'text' || current.allowFreeform) && (!readOnly || draftFor(current.id).text) && (
            <textarea
              ...
              onChange={e => {
                ...
                // Typing a custom answer clears choice selection (mutually exclusive UX).
                setDraft(current.id, {
                  text,
                  selectedOptionIds: text.trim() && current.inputType === 'choice' ? [] : draftFor(current.id).selectedOptionIds,
                });
              }}
              className={`w-full px-3.5 py-2.5 text-sm bg-surface-overlay border border-border-default rounded-xl text-fg-primary placeholder:text-fg-tertiary focus:outline-none resize-y ${readOnly ? 'cursor-default opacity-80' : 'focus:ring-1 focus:ring-brand-500/50'}`}
            />
          )}
        </div>
        ...
        <div className="flex items-center justify-between gap-2 px-6 py-4 border-t border-border-default shrink-0">
          ...
          <div className="flex items-center gap-2">
            {multiQuestion && !isLast ? (
              <button
                disabled={!readOnly && !isAnswered(current)}
