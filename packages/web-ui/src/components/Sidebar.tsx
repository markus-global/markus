      ...
      <div className={`flex items-center ${collapsed ? 'px-2 py-3.5 justify-center' : 'px-4 h-14 justify-between'}`}>
        {collapsed ? (
          <div className="flex flex-col items-center gap-2">
            <button onClick={onToggleCollapse} title={t('sidebar.expand')} className="group">
          ...
          <>
            <button onClick={onToggleCollapse} className="flex items-center gap-2.5 min-w-0 group" title={t('sidebar.collapse')}>
        ...
        {authUser && (
          collapsed ? (
            <button
              ...
              title={`${authUser.name ?? authUser.email} — ${t('common.signOut')}`}
