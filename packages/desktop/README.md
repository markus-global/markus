...
| macOS | arm64 | dmg, zip | ✅ |
| macOS | x64 | dmg, zip | ✅ |
| macOS (Mac App Store) | universal | mas, pkg | ✅ |
...
3. `build-desktop-mas` — Mac App Store 构建（仅正式版）
4. `github-release` — 上传所有产物到 GitHub Release
...
使用 macOS 原生 `hiddenInset` 样式，通过 CSS 注入实现：
- 侧边栏顶部 48px padding 避让红绿灯
- `body::before` 覆盖全窗口顶部 48px 作为拖拽区域
- 所有交互元素标记 `no-drag`

### 自动更新

通过 `electron-updater` 实现，发布到 GitHub Releases：
- 正常构建自动检查更新
- MAS 版本 (`MARKUS_MAS=true`) 禁用自动更新（由 App Store 管理）
