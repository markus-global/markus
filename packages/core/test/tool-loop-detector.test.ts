...
describe('ToolLoopDetector', () => {
  ...
  it('should allow consecutive same-tool calls with different arguments', () => {
    const d = new ToolLoopDetector({
      enabled: true,
      historySize: 30,
      warningThreshold: 3,
      criticalThreshold: 5,
      detectors: { genericRepeat: true, pingPong: true, noProgress: true },
    });

    // Bulk file writes (team package creation) must not be treated as a loop.
    for (let i = 0; i < 12; i++) {
      d.record('file_write', { path: `members/role-${i}/ROLE.md` }, `written ${i}`);
    }
