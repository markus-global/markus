...
/** Bundled templates/skills dir shipped with this Markus build (not ~/.markus overrides). */
function resolveBundledSkillsDir(): string {
  return resolve(process.env['MARKUS_TEMPLATES_DIR'] ?? resolve(process.cwd(), 'templates'), 'skills');
}

/** Read skill.json version from a skill package directory. */
function readSkillVersion(skillDir: string): string | null {
  const mfPath = join(skillDir, 'skill.json');
  if (!existsSync(mfPath)) return null;
  try {
    const mf = JSON.parse(readFileSync(mfPath, 'utf-8')) as { version?: string };
    return typeof mf.version === 'string' ? mf.version : null;
  } catch {
    return null;
  }
}
...
/**
 * Local copy that wins at runtime for a builtin skill name:
 * ~/.markus/skills first, then ~/.markus/templates/skills (init seed).
 */
function resolveLocalSkillDir(name: string): string | null {
  const installed = join(homedir(), '.markus', 'skills', name);
  if (existsSync(join(installed, 'skill.json')) || existsSync(join(installed, 'SKILL.md'))) {
    return installed;
  }
  const seeded = join(homedir(), '.markus', 'templates', 'skills', name);
  if (existsSync(join(seeded, 'skill.json')) || existsSync(join(seeded, 'SKILL.md'))) {
    return seeded;
  }
  return null;
}

/** Allocate a free artifact directory slug under builder-artifacts/{typeDir}/. */
function allocateArtifactSlug(typeDir: string, preferred: string, currentName: string): string {
  const base = join(homedir(), '.markus', 'builder-artifacts', typeDir);
  let candidate = preferred;
  let n = 2;
  for (;;) {
    const dest = join(base, candidate);
    if (!existsSync(dest) || candidate === currentName) return candidate;
    candidate = `${preferred}-${n++}`;
  }
}

/** Sync skill SKILL.md YAML frontmatter `name:` with the package slug. */
function syncSkillFrontmatterName(artDir: string, newSlug: string): void {
  ...
  try {
    ...
    const updatedFm = fm.replace(/^name:\s*.*$/m, `name: ${newSlug}`);
    if (updatedFm !== fm) writeFileSync(skillMd, updatedFm + body, 'utf-8');
  } catch (err) {
    log.warn('Failed to sync SKILL.md frontmatter name', { error: String(err) });
  }
}
...
export async function handleSkillsRoutes(
  ...
  url: URL,
    ...
    if (path === '/api/skills/builtin' && req.method === 'GET') {
      const builtinDir = resolveBundledSkillsDir();
      ...
      const skills = found
        .map(({ manifest, path: p }) => {
          ...
          try {
            const sjPath = resolve(builtinDir, manifest.name, 'skill.json');
            if (existsSync(sjPath)) raw = JSON.parse(readFileSync(sjPath, 'utf-8')) as Record<string, unknown>;
          } catch { /* skip */ }
          if (raw?.hidden) return null;
          const localDir = resolveLocalSkillDir(manifest.name);
          const localVersion = localDir ? readSkillVersion(localDir) : null;
          const inst = installedSkills.get(manifest.name);
          // Prefer on-disk local version for upgrade detection; registry may still be template-only.
          const installedVersion = localVersion ?? inst?.version ?? null;
          return {
            name: manifest.name,
            ...
            sourcePath: p,
            installed: !!localDir || !!inst,
            installedVersion,
            updateAvailable: !!(localVersion && compareVersions(manifest.version, localVersion) > 0),
    ...
    // Builtin skills with a newer version in this Markus build than the local copy.
    if (path === '/api/skills/updates' && req.method === 'GET') {
      const builtinDir = resolveBundledSkillsDir();
      const found = discoverSkillsInDir(builtinDir);
      const updates: Array<{
        type: 'skill';
      ...
      for (const { manifest } of found) {
        ...
        const installedVersion = readSkillVersion(localDir);
        if (!installedVersion) continue;
        if (compareVersions(manifest.version, installedVersion) <= 0) continue;
        const i18n = raw?.i18n as Record<string, { displayName?: string }> | undefined;
        updates.push({
          type: 'skill',
          name: manifest.name,
          displayName: (raw?.displayName as string | undefined) ?? i18n?.['zh-CN']?.displayName ?? manifest.name,
          description: manifest.description,
          installedVersion,
      ...
      server.json(res, 200, { updates, count: updates.length });
      return true;
    }
    ...
    if (path === '/api/builder/artifacts/save' && req.method === 'POST') {
      ...
      try {
        ...
        if (validationErrors.length > 0) {
          server.json(res, 400, { error: `Invalid manifest: ${validationErrors.join('; ')}` });
          return true;
        }
    ...
    // POST /api/builder/artifacts/:type/:name/ensure-slug — auto-convert legacy
    // Chinese / invalid folder names to a valid English kebab-case package slug.
    // Keeps the human title in displayName; renames the artifact directory.
    {
      const ensureMatch = path.match(/^\/api\/builder\/artifacts\/(agents?|teams?|skills?)\/([^/]+)\/ensure-slug$/);
      if (ensureMatch && req.method === 'POST') {
        const rawType = ensureMatch[1]!;
        const name = decodeURIComponent(ensureMatch[2]!);
        const typeDir = rawType.endsWith('s') ? rawType : rawType + 's';
        const type = (typeDir === 'agents' ? 'agent' : typeDir === 'teams' ? 'team' : 'skill') as PackageType;
        const base = join(homedir(), '.markus', 'builder-artifacts', typeDir);
        const artDir = join(base, name);
        if (!existsSync(artDir)) {
          server.json(res, 404, { error: 'Artifact not found' });
          return true;
        }
        ...
        try {
          ...
          if (existsSync(mfPath)) {
            try { manifest = JSON.parse(readFileSync(mfPath, 'utf-8')) as Record<string, unknown>; }
            catch { /* treat as empty */ }
          }

          const currentSlug = typeof manifest.name === 'string' && manifest.name.trim()
            ? manifest.name.trim()
            : name;
          const alreadyOk = isValidPackageSlug(currentSlug) && currentSlug === name && isValidPackageSlug(name);
          if (alreadyOk) {
            server.json(res, 200, {
              converted: false,
              type,
          ...
          const preferred = ensurePackageSlug(currentSlug).slug;
          const newSlug = allocateArtifactSlug(typeDir, preferred, name);
          const displayName = (typeof manifest.displayName === 'string' && manifest.displayName.trim())
            ? manifest.displayName.trim()
            : (typeof manifest.name === 'string' && !isValidPackageSlug(manifest.name) ? manifest.name : name);
          ...
          const validationErrors = validateManifest(manifest);
          if (validationErrors.length > 0) {
            // Ensure displayName/version so validate can pass after slug fix.
            if (!manifest.version) manifest.version = '1.0.0';
            const retry = validateManifest(manifest);
            if (retry.length > 0) {
              server.json(res, 400, { error: `Cannot convert slug: ${retry.join('; ')}` });
              return true;
            }
          }

          writeFileSync(mfPath, JSON.stringify(manifest, null, 2), 'utf-8');
          if (type === 'skill') syncSkillFrontmatterName(artDir, newSlug);

          let finalDir = artDir;
          if (name !== newSlug) {
            const dest = join(base, newSlug);
            renameSync(artDir, dest);
            finalDir = dest;
          }

          log.info(`Converted artifact slug ${type}/${name} → ${newSlug}`);
          server.json(res, 200, {
            converted: true,
            type,
            previousName: name,
          ...
          server.json(res, 500, { error: `Slug conversion failed: ${String(err)}` });
        }
        return true;
      }
    }
