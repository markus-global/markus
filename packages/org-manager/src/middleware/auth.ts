...
/**
 * Merge builtin multimodal tags with LiteLLM enrichment.
 * Never let an empty/incomplete LiteLLM capability list wipe builtins like
 * `imageGeneration` / `tts` (that caused false "model mismatch" in Settings).
 */
export function mergeModelCapabilities(
  builtin?: string[] | null,
  enriched?: string[] | null,
): string[] | undefined {
  const a = builtin?.filter(Boolean) ?? [];
  const b = enriched?.filter(Boolean) ?? [];
