...
export function createMockStorage(): StorageBridge {
  ...
  return {
    ...
    chatSessionRepo: {
      ...
      getOrCreateMainSession: vi.fn(() => ({ id: 'sess-main-1', title: 'Main', isMain: true, agentId: AGENT_A })),
      appendMessage: vi.fn(async () => ({ id: 'msg-1' })),
      updateLastMessage: vi.fn(() => {}),
      updateSessionMetadata: vi.fn(() => {}),
      getSessionMetadata: vi.fn(() => null),
      getSession: vi.fn((sessionId: string) => ({
        id: sessionId, agentId: AGENT_A, userId: 'anonymous', title: 'Chat', isMain: false,
      })),
      getMessages: vi.fn(async () => ({ messages: [{ id: 'm1', role: 'user', content: 'Hi' }], hasMore: false })),
