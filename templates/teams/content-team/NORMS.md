...
| 环节 | 质量要求 |
|------|---------|
| 选题 → 创作 | 选题有明确角度、目标平台、受众定义 |
| 创作 → 审核 | 初稿完整、事实可追溯、符合平台格式 |
| 审核 → 发布 | 编辑确认准确性、调性一致性、合规性 |
| 发布 → 复盘 | 发布链接已记录、数据追踪已配置 |

## 错误恢复

- **事实错误**：发现后立即通知编辑，评估是否需要更正或撤回。在 `task_note` 中记录错误原因和修正方案。
- **平台违规**：内容被平台警告或删除时，分析原因，更新内容规范，避免再犯。
- **数据异常**：发布后数据表现远低于预期时，分析选题、标题、发布时间等因素，记录到团队知识库。
