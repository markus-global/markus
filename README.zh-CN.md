...
不同于将任务分派给外部 CLI 工具的智能体编排器，**Markus 内置了完整的智能体运行时**。每个智能体直接调用 LLM API，使用内置工具（Shell、文件、Git、Web 搜索、代码分析、MCP），维护跨会话持久记忆，并通过心跳机制主动运作 — 即使你不在线。
...
### 三层记忆系统（Tulving）
程序性记忆（ROLE.md + 技能）、语义记忆（MEMORY.md + 观察缓冲）、情景记忆（对话历史）。智能体持续积累知识并自动整合 — 上下文跨重启持久保留，而非仅存在于单次对话。
