...
|----|----------|------|------|-------------|--------|
| T-001 | Low | [DESIGN] | `core/src/llm/router.ts` | Providers without `getCapabilities()` bypass capability filter for non-text tasks — they are included as fallbacks even when they may not support the modality | Open |
...
| T-006 | Low | [DESIGN] | `core/src/tools/file.ts` | `denyMessage` is exported but unused internally — write/edit tools inline their own denial messages creating duplication | Open — low priority |
...
| T-011 | High | [RACE] | `core/src/llm/auth-profiles.ts` | `acquireLock` uses `writeFileSync('wx')` but stale-lock check (>30s) races with other processes — two processes can both decide the lock is stale and both acquire it simultaneously | Open |
...
| T-014 | Medium | [DESIGN] | `core/src/agent.ts` | 11 empty `catch {}` blocks + 4 `.catch(() => {})` — errors in consolidation, session finalization, and cleanup are silently dropped; at minimum these should log at debug level | Open — remaining catches are in agent enrichment code |
| T-015 | Low | [BUG] | `core/src/agent.ts` | `updateTokensUsed` adds to `state.tokensUsedToday` AND delegates to `stateManager.updateTokensUsed` — if stateManager also accumulates, tokens could be double-counted depending on which source is read | Open |
...
| T-017 | Low | [DESIGN] | `core/src/agent.ts` | Event listener added in constructor (`on(...)`) but never removed in `stop()` — if agent is created/destroyed repeatedly, listeners accumulate. Only 1 listener currently but pattern is risky | Open |
...
| T-019 | Low | [BUG] | `core/src/tools/web-search.ts` | `_dispatcher` module-level cache uses `false` as sentinel for "not yet resolved" — but `resolveProxyDispatcher` sets it to `undefined` on no proxy, which is falsy. Second call re-resolves because `_dispatcher !== false` passes but `_dispatcher || undefined` always returns undefined. Actually correct, but confusing — the `||` operator means `_dispatcher = undefined` will return `undefined` correctly, and `_dispatcher = new ProxyAgent()` will return the agent. The `false` sentinel works but is fragile | Open — not a bug, but fragile pattern |
...
| T-022 | Low | [DESIGN] | `core/src/attention.ts` | 6 empty catch blocks — attention/triage failures are swallowed; if the LLM returns malformed JSON for priority assessment, the item falls through to default handling without any logging | Open |
...
| T-026 | Low | [DESIGN] | `core/src/memory/store.ts` | 5 empty catch blocks — memory persistence failures (write daily log, compact session) are swallowed; data loss in memory subsystem is invisible | Open |
...
- T-010: Config file concurrent write race ✅ (fixed with `O_EXCL` filesystem lock)
- T-011: Auth profile lock race — use OS-level advisory locks
- T-012: SQLite transactions — wrap multi-step operations in `db.transaction()`

### Medium Priority (silent error swallowing)
- T-013: Replace empty catch blocks with logging ✅ (partially fixed — 5 critical catches in agent-manager)
- T-014: Same for agent.ts — remaining catches are enrichment-level (low risk)
- T-016: Add mutex/queue for `handleMessage` to prevent session corruption
- T-021: task-tools catches are for non-fatal enrichment — acceptable as-is
- T-024: Metrics persistence logging ✅

### Low Priority (design improvements)
- T-001, T-002, T-004, T-006, T-007, T-017, T-022, T-023, T-024, T-026
